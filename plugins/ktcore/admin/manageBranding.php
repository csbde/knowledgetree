...
class ManageBrandDispatcher extends KTAdminDispatcher 
{
    ...
    function saveConfiguration($logoTitle, $logoPath)
    {
        $config = KTConfig::getSingleton();
    ...
    function uploadLogo($fileDetails)
    {
        global $default;
        $config = KTConfig::getSingleton();
        ...
        $url = $storage->getStoragePath($logoFile);
        $storage->setAllAccess($logoFile);
        return $url;
    }

    function isSupportedExtension($extension = '', $mimeType = '')
    {
    	if (empty($extension)) {
