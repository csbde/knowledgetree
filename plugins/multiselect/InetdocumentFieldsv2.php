...
class InetDocumentFieldDispatcher extends KTAdminDispatcher {
     ...
     * iNET Process
     */
    public function do_newfieldset()
    {
    ...
    public function do_create()
    {
        ...
        if (!empty($data['name'])) {
            ...
            if (!PEAR::isError($oFieldset)) {
                ...
                $extraErrors['name'] = _kt('There is already a fieldset with that name.');
            }
        }
        ...
        if (!empty($errors) || !empty($extraErrors)) {
            return $oForm->handleError(null, $extraErrors);
        }

        // we also need a namespace.
        $tempName = sanitizeForHTML($data['name']);
        $namespace = KTUtil::nameToLocalNamespace('fieldsets', $tempName);
	    ...
	    'isConditional' => $isConditional,   // handle this
     ...
     * iNET Process
     */
    public function do_editfieldset()
    {
     ...
     * iNET Process
     */
    public function do_savefieldset()
    {
        $oForm = $this->form_edit();
        $res = $oForm->validate();
        ...
        if (sanitizeForHTML($data['name']) != $this->oFieldset->getName()) {
            ...
            if (!PEAR::isError($oOldFieldset) && $oOldFieldset->getName() == sanitizeForHTML($data['name'])) {
                $extraErrors['name'][] = _kt('A fieldset with that name already exists.');
            }
        }
