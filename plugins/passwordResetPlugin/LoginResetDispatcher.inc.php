...
class LoginResetDispatcher extends KTDispatcher {
    ...
    public function do_main()
    {
        ...
        $resetPassword = $this->checkReset();

        KTUtil::save_base_kt_url();

        if ($user instanceof User) {
            $res = $this->performLogin($user);
            if ($res) {
                $user = array($res);
            }
        }
        ...
        if (is_array($user) && count($user)) {
            ...
            foreach ($user as $error) {
                $_REQUEST['errorMessage'][] = $error->getMessage();
            }
        ...
        $i18nRegistry =& KTi18nregistry::getSingleton();
        $registeredLangs = $i18nRegistry->geti18nLanguages('knowledgeTree');
        $languageNames = $i18nRegistry->getLanguages('knowledgeTree');
        $registeredLanguageNames = array();

        if (!empty($registeredLangs)) {
            foreach (array_keys($registeredLangs) as $lang) {
                $registeredLanguageNames[$lang] = $languageNames[$lang];
            }

            asort($registeredLanguageNames);
        }

        // extra disclaimer, if plugin is enabled
        $pluginRegistry =& KTPluginRegistry::getSingleton();
        $plugin =& $pluginRegistry->getPlugin('ktstandard.disclaimers.plugin');
        if (!PEAR::isError($plugin) && !is_null($plugin)) {
            $disclaimer = $plugin->getLoginDisclaimer();
        }
        ...
        if (ACCOUNT_ROUTING_ENABLED && $default->tier == 'trial') {
            ...
            $page->setBodyOnload("javascript: ktOlark.popupTrigger('Hi, do you need help logging in?', 30000);");
        }
        ...
        $useEmail = $config->get('user_prefs/useEmailLogin', false);

        // Include additional js and css files if plugin
        $plugin =& $pluginRegistry->getPlugin('password.reset.plugin');
        if ($plugin != null) {
            $js[] = $this->getAdditionalJS($plugin, $useEmail);
            $css[] = $plugin->getURLPath('resources/passwordReset.css');
        }
        ...
        $templateData = array(
            ...
            'languages' => $registeredLanguageNames,
            'selected_language' => $default->defaultLanguage,
            ...
            'use_email' => $useEmail,
            'new_email' => $this->getEmail($useEmail),
    ...
    protected function getAdditionalJS($plugin, $useEmail = false)
    {
        if ($useEmail) {
            ...
            return $plugin->getURLPath('resources/passwordReset.js');
        }
    }

    protected function getEmail($useEmail = false)
    {
        $email = false;
        if ($useEmail) {
    ...
    protected function simpleRedirectToMain($errorMessage, $url, $params)
    {
    ...
    public function do_login()
    {
        ...
        if (PEAR::isError($user) || ($user === false)) {
            if ($user instanceof ktentitynoobjects) {
                $this->handleUserDoesNotExist($username, $password, $extra);
            }
        ...
        if ($authenticated !== true) {
            $this->doFailedLoginRedirect($url, $queryParams);
        }
    ...
    public function do_providerVerify()
    {
        ...
        $authProvider =& KTAuthenticationUtil::getAuthenticationProviderForUser($this->oUser);
        $authProvider->subDispatch($this);
        exit(0);
    }
     ...
     * @return unknown
     */
    protected function performLogin(&$user, $url = '', $doRedirect = true)
    {
        $session = new Session();
    ...
    protected function handleUserDoesNotExist($username, $password, $extra = null)
    {
        if (empty($extra)) {
            $extra = array();
        }
        ...
        $KTConfig = KTConfig::getSingleton();
        $allow = $KTConfig->get('session/allowAutoSignup', true);

        if ($allow) {
            $res = KTAuthenticationUtil::autoSignup($username, $password, $extra);
            if (empty($res)) {
                return $res;
            }
    ...
    public function do_autoSignup()
    {
        $authSource =& $this->oValidator->validateAuthenticationSource($_REQUEST['source_id']);
        $authProvider =& KTAuthenticationUtil::getAuthenticationProviderForSource($authSource);
        $dispatcher = $authProvider->getSignupDispatcher($authSource);
        $dispatcher->subDispatch($this);
        exit(0);
    }
    ...
    public function do_checkCookie() {
        ...
        if ($cookieTest !== $cookieVerify) {
            Session::destroy();
            $this->simpleRedirectToMain(
                                        _kt('You must have cookies enabled to use the document management system.'),
                                        $url,
                                        $queryParams
            );
            exit(0);
    ...
    protected function validateCredentials($email, $user)
    {
        $KTConfig = KTConfig::getSingleton();
        if ($KTConfig->get('user_prefs/useEmailLogin', false)) {
    ...
    protected function validateEmailUser($email)
    {
        // Check that the user and email match up in the database
        $query = 'SELECT id FROM users WHERE username = ? AND email = ?';
        $params = array($email, $email);
        return DBUtil::getOneResultKey(array($query, $params), 'id');
    }

    protected function validateUser($email, $user) // This may go private, if not used by child class.
    {
        // Check that the user and email match up in the database
        $query = 'SELECT id FROM users WHERE username = ? AND email = ?';
    ...
    protected function sendResetEmail($id, $email)
    {
        ...
        $mailer = new Email();
        $res = $mailer->send($email, $subject, $body);
    ...
    protected function resetPasswordEmailUser($email, $password)
    {
        // Get user from db
        $query = 'SELECT id FROM users WHERE username = ? AND email = ?';
        $params = array($email, $email);
        $id = DBUtil::getOneResultKey(array($query, $params), 'id');
        if (!is_numeric($id) || $id < 1) {
            return _kt('Please check that you have entered a valid email address.');
        }

        return $this->sendUpdatePasswordAndEmail($id, false, md5($password));
    }

    protected function sendUpdatePasswordAndEmail($id, $email, $password)
    {
        // Check expiry
        $expiry = KTUtil::getSystemSetting('password_reset_expire-' . $id);
        if ($expiry < time()) {
    ...
    // TODO Test this function and the one in the resetEmailDispatcher.
