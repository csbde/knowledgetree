...
PermissionsTable.prototype = {
    'initialize' : function(sLookupName, sEntityName, permissions) {
	bindMethods(this);

	this.permissions = permissions;

	this.oLookup = getJSONLookupWidget(sLookupName);
	this.oLookup.sAction += '&' + queryString({'permissions':map(function(a){return a['id'];}, permissions)});
...
function initializeTable(permissions) {
...
function initializePermissions(sName, sAction, permissions) {
    addLoadEvent(function() {
		     initJSONLookup(sName, sAction)();
		     initializeTable(permissions);
		 });
}
