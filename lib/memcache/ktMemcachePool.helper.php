				...
				if (is_array($server)) {
					if (isset($server['url']) && isset($server['port'])) {
						...
						if (@$_test->connect($host, $port)) {
							...
							$this->memcache->addServer($server['url'], $server['port'], self::PERSIST, self::WEIGHT, self::TIMEOUT, self::RETRY_INTERVAL);
						}
						else {
	...
	private function _set($key, $value)
	{
		...
		if (self::SELF_REPLICATION) {
			...
			if ($this->get($key) == false) {
				$ret = $this->memcache->set($key, $value, self::COMPRESS,50);
			}
			else {
				$ret = $this->memcache->replace($key, $value);
			}
		}
