...
class KTDocumentCore extends KTEntity {
    ...
    function getRealDocumentId(){
    	...
    	if(PEAR::isError($realDocument)){
            $GLOBALS['default']->log->error("Couldn't get linked document ({$this->getLinkedDocumentId()}) for document ({$this->getId()}): {$realDocument->getMessage()}");
    ...
    function getRealDocument()
    {
        ...
        if(PEAR::isError($document)){
            $GLOBALS['default']->log->error("Couldn't get linked document ({$this->getLinkedDocumentId()}) for document ({$this->getId()}): {$document->getMessage()}");
