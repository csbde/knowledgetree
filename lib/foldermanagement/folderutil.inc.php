...
class KTFolderUtil {
    ...
    function _add($oParentFolder, $sFolderName, $oUser)
    {
        ...
        $res = $storageManager->createFolder($oFolder);
        if (PEAR::isError($res)) {
            $oFolder->delete();
            return $res;
        }
    ...
    function move($oFolder, $oNewParentFolder, $oUser, $sReason = null, $bulk_action = false)
    {
        ...
        $res = $storageManager->moveFolder($oFolder, $oNewParentFolder);
        if (PEAR::isError($res)) {
            return $res;
        }
    ...
    function rename($oFolder, $sNewName, $oUser)
    {
        ...
        $res = $storageManager->renameFolder($oFolder, $sNewName);
        if (PEAR::isError($res)) {
            return $res;
        }
    ...
    *   - step-by-step delete.
    */
    function delete($startFolder, $user, $reason, $options = null, $bulkAaction = false)
    {
        require_once(KT_LIB_DIR . '/unitmanagement/Unit.inc');
        $storageManager = KTStorageManagerUtil::getSingleton();
        $permission = KTPermission::getByName('ktcore.permissions.delete');
        $ignorePermissions = KTUtil::arrayGet($options, 'ignore_permissions');
        $folderIds = array(); // of oFolder
        $documents = array(); // of oDocument
        $failedDocuments = array(); // of String
        $failedFolders = array(); // of String
        $remainingFolders = array($startFolder->getId());

        DBUtil::startTransaction();

        while (!empty($remainingFolders)) {
            $folderId = array_pop($remainingFolders);
            $folder = Folder::get($folderId);
            if (PEAR::isError($folder) || ($folder == false)) {
                DBUtil::rollback();
                return PEAR::raiseError(sprintf(_kt('Failure resolving child folder with id = %d.'), $folderId));
            }

            $unit = Unit::getByFolder($folder);
            if (!empty($unit)) {
                DBUtil::rollback();
                return PEAR::raiseError(sprintf(_kt('Cannot remove unit folder: %s.'), $folder->getName()));
            }

            // don't just stop ... plough on.
            if (!$ignorePermissions && !KTPermissionUtil::userHasPermissionOnItem($user, $permission, $folder)) {
                $failedFolders[] = $folder->getName();
            }
            else {
                $folderIds[] = $folderId;
            }
            ...
            // child documents
            $childDocs = Document::getList(array('folder_id = ?',array($folderId)));
            foreach ($childDocs as $doc) {
                if (!$ignorePermissions && $doc->getImmutable()) {
                    if (!KTBrowseUtil::inAdminMode($user, $startFolder)) {
                        $failedDocuments[] = $doc->getName();
                        continue;
                    }
                }

                if ($ignorePermissions || (KTPermissionUtil::userHasPermissionOnItem($user, $permission, $doc) && KTDocumentUtil::canBeDeleted($doc)) ) {
                    $documents[] = $doc;
                }
                else {
                    $failedDocuments[] = $doc->getName();
            ...
            // child folders.
            $aCFIds = Folder::getList(array('parent_id = ?', array($folderId)), array('ids' => true));
            $remainingFolders = kt_array_merge($remainingFolders, $aCFIds);
        }

        // FIXME we could subdivide this to provide a per-item display (viz. bulk upload, etc.)

        if ((!empty($failedDocuments) || (!empty($failedFolders)))) {
            $sFD = '';
            $sFF = '';
            if (!empty($failedDocuments)) {
                $sFD = '<b>'._kt('Documents: ') .'</b>' . implode(', ', $failedDocuments) . '. ';
            }
            if (!empty($failedFolders)) {
                $sFF = '<b>'._kt('Folders: ') .'</b>' . implode(', ', $failedFolders) . '.';
            }
        ...
        foreach ($documents as $document) {
            $res = KTDocumentUtil::delete($document, $reason, null, true);
            if (PEAR::isError($res)) {
                DBUtil::rollback();
                return PEAR::raiseError(_kt('Delete Aborted. Unexpected failure to delete document: ') . $document->getName() . $res->getMessage());
            }
        }

        $storageManager->removeFolderTree($startFolder);

        // Check for symbolic links to the folder and its sub folders
        $aSymlinks = array();
        foreach($folderIds as $iFolder) {
            $folder = Folder::get($iFolder);
            $aLinks = $folder->getSymbolicLinks();
            $aSymlinks = array_merge($aSymlinks, $aLinks);
        }

        // documents all cleared.
        $sQuery = 'DELETE FROM ' . KTUtil::getTableName('folders') . ' WHERE id IN (' . DBUtil::paramArray($folderIds) . ')';
        $aParams = $folderIds;
        ...
        if ($reason !== null) {
            $sComment .= sprintf(_kt(" (reason: %s)"), $reason);
        }

        $oTransaction = KTFolderTransaction::createFromArray(array(
            'folderid' => $startFolder->getId(),
            'comment' => _kt('Folder deleted'),
            'transactionNS' => 'ktcore.transactions.delete',
            'userid' => $user->getId(),
            'ip' => Session::getClientIP(),
            'parentid' => $startFolder->getParentID(),
        ));
    ...
    function copy($oSrcFolder, $oDestFolder, $oUser, $sReason, $sDestFolderName = NULL, $copyAll = true)
    {
        ...
        $res = $storageManager->createFolder($oNewBaseFolder);
        if (PEAR::isError($res)) {
        ...
        while (!empty($aRemainingFolders) && $copyAll) {
            ...
            $id = DBUtil::autoInsert($sTable, $aRow);
            if (PEAR::isError($id)) {
                $storageManager->removeFolder($oNewBaseFolder);
                DBUtil::rollback();
                return $id;
            }
            ...
            $res = $storageManager->createFolder($oNewFolder);
            if (PEAR::isError($res)) {
