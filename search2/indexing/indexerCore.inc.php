...
abstract class Indexer
{
     ...
     */
    protected function logPendingDocumentInfoStatus($documentId, $message, $level)
    {
...
//        $oSolr = new RestSolr('10.33.15.255', '8984', '/solr/'); //connecting to a dynamic core (core still needs a folder struct on the server though)
        ...
        if ($extract && empty($extractorClass))
        {
            /*
        ...
        else
        {
            ...
            */
            if ($extract && !$this->isExtractorEnabled($extractorClass))
            {
        ...
        if ($indexDocument)
        {
            ...
            if (!$extract || $extractor->extractTextContent())
            {
                if ($extract) {
                ...
                if ($indexDiscussion)
                {
                    if ($extract && !$this->filterText($targetFile))
                    {
